...
uint256 hashGenesisBlock("0xdc9eaa46eefe65f1b045fdc0e0c3283401cd6707afb4bb259be5ea9b7c64fe4f");
static CBigNum bnProofOfWorkLimit = CBigNum().SetCompact(504365644); // Kktcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kktcoin Signed Message:\n";
...
{
    // Kktcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
   ...
   int64 coin = 100000000;
    ...
    if(nHeight > 1 && nHeight <= 100 ){
		...
		return  nSubsidy = 10 * coin  + nFees;
		
	}
	
	if(nHeight == 1 ){
		
	 return 	nSubsidy = 49999999 * coin  + nFees;
		                   
	}
	
	
    // Subsidy is cut in half every 1000000 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 1000000); // Kktcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 302400; // Kktcoin: 3.5 days
static const int64 nTargetSpacing = 150; // Kktcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kktcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kktcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Kktcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x39f0319aecfe9e6b8728466ada4a5c67f1aefab2ab84acd6a6c1be888f3c5c4b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("b48193d33b7780084f213eb16e01b3aaecc83ceb4f5f951292485b6abc520335e77cdf045b84ded42aaf0c4e8be8cae39fc04ffac5b9c40a23b4ca13ab12e1c162") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517828794;
        block.nBits    = 504365644;
        block.nNonce   = 3122741732;

        if (fTestNet)
        {
            block.nTime    = 1517828794;
            block.nNonce   = 3417311746;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xcee323a82c06388f17664907e99994cee04ce9eea76ecaff175aae3e1d738c15"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Kktcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KktcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KktcoinMiner : ProcessBlock, block not accepted");
    }
